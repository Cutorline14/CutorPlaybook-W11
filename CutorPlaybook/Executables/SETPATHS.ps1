...
$rootPath = "HKLM:\SOFTWARE\Cutor\Services"
...
foreach ($key in $registryKeys) {
    ...
    Write-Output($path)
    if ($path -notlike "$windir\CutorDesktop\*") {
        $marker = "CutorDesktop\"
        $index = $path.IndexOf($marker)
        $result = $path.Substring($index + $marker.Length)
        Set-ItemProperty -Path $key.PSPath -Name $valueName -Value "$windir\CutorDesktop\$result"
    }
}
