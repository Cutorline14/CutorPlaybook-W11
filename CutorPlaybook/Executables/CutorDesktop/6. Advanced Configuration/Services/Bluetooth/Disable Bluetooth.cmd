...
for %%a in (
	...
	call "%windir%\CutorModules\Scripts\setSvc.cmd" %%~a 4
)

:: Seems to not exist sometimes
call "%windir%\CutorModules\Scripts\setSvc.cmd" BthPan 4 > nul 2>&1

:: Disable Bluetooth devices
call "%windir%\CutorModules\Scripts\toggleDev.cmd" -Silent '*Bluetooth*'

:: Disable in Send To context menu
call "%windir%\CutorDesktop\4. Interface Tweaks\Context Menus\Send To\Debloat Send To Context Menu.cmd" -Disable @('Bluetooth')
