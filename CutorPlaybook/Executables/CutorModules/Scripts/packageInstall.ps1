...
param (
	...
	[string]$PackagesPath = "$([Environment]::GetFolderPath('Windows'))\CutorModules\Packages",
...
$safeModePackageList = "$sys32\safeModePackagesToInstall.cutormodule"
...
"@
	...
	if ($failedPackages.Count -gt 0) {
		...
		if ($NoInteraction) {
			...
			$failedMsgTitle = 'CutorFailedComponentMsgBox'
			$failedMsgArgs = "/c title Finalizing Installation - Cutor & echo Do not close this window. & schtasks /delete /tn `"$failedMsgTitle`" /f > nul & " `
		...
		if ($safeModeStatus) {
			Write-Host "Please report this to the Cutor team, as there's no automatic fallbacks past Safe Mode." -ForegroundColor Magenta
...
if ($SafeMode) {
	...
	if ($matchedPackages.Count -le 0) {
		Write-Host "[ERROR] Safe Mode package list not found! Please report this to Cutor." -ForegroundColor Red
		ExitSafeModePrompt
	}

	$packagesThatDontExist = $matchedPackages | ForEach-Object { if (!(Test-Path $_ -PathType Leaf)) { $_ } }
	if ($packagesThatDontExist) {
		Write-Host "[ERROR] Some Safe Mode packages weren't found. Please report this to Cutor." -ForegroundColor Red
...
Would you like Cutor to restart your system into Safe Mode and try again? This process shouldn't take much time.

Please note that if you chose to disable Windows Defender, it may still remain enabled if you select 'No'. However, you can always try disabling it later in the Cutor folder.
"@

	if ((Read-MessageBox -Title "Cutor - Component Modification" -Body $body -Icon Question) -eq 'Yes') {
		SafeMode -Enable
		Restart
	}
...
function MakeRepairSource {
	...
	$srcPath = "%SystemRoot%\CutorModules\Packages\WinSxS"
