...
$packageInstall = "$windir\CutorModules\Scripts\packageInstall.ps1"
...
$package = "*Z-Cutor-NoTelemetry-Package*"
...
function Menu {
	...
	Write-Host "This script adds or removes Cutor' NoTelemetry package, which removes certain telemetry components in Windows.`n" -ForegroundColor Cyan

	Write-BulletPoint @"
Removing the package restores the telemetry components, which can aid in troubleshooting. If
   resolving the issue involves removing the package, please report this as an issue to Cutor.`n
"@
	Write-BulletPoint @"
Cutor retains policies that should disable telemetry even after the package removal. These policies
