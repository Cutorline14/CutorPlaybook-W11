...
# Backup Cutor Services and Drivers
$windir = $([Environment]::GetFolderPath('Windows'))
function Backup-CutorServices {
$filePath = "$([Environment]::GetFolderPath('Windows'))\CutorModules\Other\cutorServices.reg"
...
& "$windir\CutorModules\Scripts\settingsPages.cmd" /hide account
...
function Disable-CoreIsolation {
    & ".\CutorModules\Scripts\ScriptWrappers\ConfigVBS.ps1" -DisableAllVBS
}
...
function Disable-Mitigations {
    Start-Process -FilePath "CutorDesktop\7. Security\Mitigations\Disable All Mitigations.cmd" -ArgumentList "/silent" -NoNewWindow -Wait
}
...
# Set Cutor Default Profile Pictures
...
function Set-PowerSettings {
    ...
    if ($DisablePowerSaving) {
        Start-Process -FilePath "CutorDesktop\3. General Configuration\Power-saving\Disable Power-saving.cmd" -ArgumentList "/silent" -NoNewWindow -Wait
    }

    if ($DisableHibernation) {
        Start-Process -FilePath "CutorDesktop\3. General Configuration\Hibernation\Disable Hibernation (default).cmd" -ArgumentList "/silent" -NoNewWindow -Wait
    }
