...
# Function to add and set the Cutor themes by default
function Set-CutorTheme {
    & "$windir\CutorModules\initPowerShell.ps1"
    Set-Theme -Path "$([Environment]::GetFolderPath('Windows'))\Resources\Themes\cutor-dark2.theme"
    Set-ThemeMRU
    ...
    reg add "HKCU\Software\Policies\Microsoft\Windows\Personalization" /v "ThemeFile" /t REG_SZ /d "%windir%\Resources\Themes\cutor-dark2.theme" /f
}
...
function Remove-ExtractFromContextMenu {
    & "$windir\CutorDesktop\4. Interface Tweaks\Context Menus\Extract\Remove Extract (default).cmd" /justcontext
}

# Function to remove printing from context menus
function Remove-PrintingFromContextMenus {
   & "$windir\CutorDesktop\6. Advanced Configuration\Services\Printing\Disable Printing.cmd" /justcontext
}
...
function Set-SendToContextMenu {
    & "$windir\CutorDesktop\4. Interface Tweaks\Context Menus\Send To\Debloat Send To Context Menu.cmd" -Disable @('Documents', 'Mail Recipient', 'Fax recipient', 'Bluetooth')
}
...
function Hide-GalleryInFileExplorer {
    & "$windir\CutorDesktop\4. Interface Tweaks\File Explorer Customization\Gallery\Disable Gallery (default).cmd" /justcontext
}
...
function Disable-NetworkNavigationPaneInExplorer {
    & "$windir\CutorDesktop\3. General Configuration\File Sharing\Network Navigation Pane\Disable Network Navigation Pane (default).cmd" /justcontext
}
...
function Restore-OldContextMenu {
    & "$windir\CutorDesktop\4. Interface Tweaks\Context Menus\Windows 11\Old Context Menu (default).cmd" /justcontext
}
...
function Add-NetworkSharingShortcut {
    ...
    New-Shortcut -Source 'control.exe' `
        -Destination "$([Environment]::GetFolderPath('Windows'))\CutorDesktop\3. General Configuration\File Sharing\Sharing Settings.lnk" `
...
function Set-StartMenu {
    ...
    foreach ($userKey in (Get-RegUserPaths).PsPath) {
        ...
        if ([string]::IsNullOrEmpty($appData) -or !(Test-Path $appData)) {
            ...
            Copy-Item -Path "$windir\CutorModules\Other\Layout.xml" -Destination "$appdata\Microsoft\Windows\Shell\LayoutModification.xml" -Force
...
function Disable-AutomaticStoreAppArchiving {
    & "$windir\CutorDesktop\3. General Configuration\Store App Archiving\Disable Store App Archiving (default).cmd" /justcontext
}
